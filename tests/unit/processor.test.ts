...
describe("Processor", () => {
  ...
  it("should chain multiple operations", () => {
    ...
    expect(mockFunctions.addTrimOperation).toHaveBeenCalledWith({
      start: 10,
      end: 20,
    });
    expect(mockFunctions.addResizeOperation).toHaveBeenCalledWith({
      width: 640,
      height: 480,
    });
    expect(mockFunctions.addCropOperation).toHaveBeenCalledWith(
      300,
      300,
      10,
      10,
    );
    expect(mockFunctions.setOutput).toHaveBeenCalledWith(
      "output.mp4",
      undefined,
    );
  });
